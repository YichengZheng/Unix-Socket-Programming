...
const unsigned short PORT_SERVER = 23157; // Port number for Server B
...
int main(){
    ...
    /*---------------------------------------
     Create UDP socket
     -----------------------------------------*/
    int socket_c = socket(AF_INET, TYPE_UDP, 0);// Beej code
    
    if (socket_c < 0) {
        perror("socket C cannot be created");
        close(socket_c);
        return 0;
    }
    ...
    int bindFlag = :: bind(socket_c, (struct sockaddr*)&server_addr, sizeof(server_addr));// Beej code
    if (bindFlag < 0) {
        perror("socket C cannot be bound ");
        close(socket_c);
        return 0;
    }else{
        std::cout<<"The Server C is up and running using UDP on port "
    ...
    std::ifstream file("backendC.txt");
    std::string line;
    int lineNum = 0;
    if (!file.is_open()){
        perror ("backendC dictionary file cannot be opened");
        return 0;
    }else {
        while (!file.eof()){
    ...
    while (true) {
        ...
        int recvFlagFcn = recvfrom(socket_c, function, 101, 0, (struct sockaddr *)&client_addr, &addrlen);// Beej code
        int recvFlagWord = recvfrom(socket_c, word, 101, 0, (struct sockaddr *)&client_addr, &addrlen);// Beej code
        if(recvFlagFcn < 0 || recvFlagWord < 0 ){
            perror("Error in receiving messages from AWS");
            close(socket_c);
            return 0;
        }
        std::cout<< "The Server C received input "<< word << " and operation "<< function<< std::endl;
        ...
        if (compareFlagSear == 0){
            ...
            std::cout << "The Server C has found " << std::to_string(numMatches) << " matches and " << std::to_string(numOneEdit) << " similar words" << std::endl;
            /*---------------------------------------
             Send output results to AWS
             -----------------------------------------*/
            int total = numMatches + numOneEdit;
            sendResult(socket_c, total, client_addr, addrlen, exactIndex, wordIndex, keyword);
            
            if (total != 0){
                // send numOneEdit to AWS
                sendto(socket_c ,std::to_string(numOneEdit).c_str(), 101,
            ...
            if (numOneEdit !=0){
                ...
                sendto(socket_c, oneEditWord.c_str(), 101,
                       0, (struct sockaddr *)&client_addr, addrlen );
                sendto(socket_c, oneEditdef.c_str(), 101,
                       0, (struct sockaddr *)&client_addr, addrlen );
        ...
        if (compareFlagPre == 0){
            ...
            std::cout << "The Server C has found "<< std::to_string(total) << " matches" << std::endl;
            /*---------------------------------------
             Send output results to AWS
             -----------------------------------------*/
            sendResult(socket_c, total, client_addr, addrlen, exactIndex, wordIndex, keyword);
...
/*---------------------------------------
 ...
 -----------------------------------------*/
void sendResult(int socket_c, int total, struct sockaddr_in client_addr, socklen_t addrlen, std::vector<int> exactIndex, std::vector<int> wordIndex, std::vector<std::string> keyword){
    // convert int total to string
    std::string tot = std::to_string(total);
    // send to AWS
    sendto(socket_c ,tot.c_str(), 101,
           0, (struct sockaddr *)&client_addr, addrlen );// Beej code
    if(total != 0){
        for (int i = 0; i < exactIndex.size(); i++){
            sendto(socket_c ,keyword[exactIndex[i]].c_str(), 101,
                   0, (struct sockaddr *)&client_addr, addrlen );// Beej code
        }
        for (int i = 0; i < wordIndex.size(); i++){
            sendto(socket_c ,keyword[wordIndex[i]].c_str(), 101,
                   0, (struct sockaddr *)&client_addr, addrlen );// Beej code
        }
    }
    std::cout << "The Server C finished sending the output to AWS"<< std::endl;
}
